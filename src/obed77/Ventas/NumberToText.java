...
public class NumberToText {
    ...
    public String Convertir(String numero, boolean mayusculas) {
        ...
        if (Pattern.matches("\\d{1,15},\\d{1,2}", numero)) {
            ...
            if (Long.parseLong(Num[0]) == 0) {//si el valor es cero
                literal = "cero ";
            } else if (Long.parseLong(Num[0]) > 999999999) {//si es mil millon
    ...
    private String getCentenas(String num) {// 999 o 099
        if( Long.parseLong(num)>99 ){//es centena
            if (Long.parseLong(num) == 100) {//caso especial
                return " cien ";
            } else {
            ...
            return getDecenas(Long.parseLong(num)+"");            
        }        
    }
...
private String getMilMillones(String numero) { //000 000 000        
        //se obtiene los millones
        String millon = numero.substring(numero.length() - 9);
        //se obtiene los milmillones
        String milmillon = numero.substring(0, numero.length() - 9);
        String n = "";
        if(milmillon.length()>1){
            n = getCentenas(milmillon) + "mil millones ";
        }else{
            n = getUnidades(milmillon) + "mil millones ";
        }
